...
// DONE: Implement the following function to check if the order belongs to current loggedIn customer (order.userId equals or not to req.user.id)/restaurantor
const checkOrderCustomer = async (req, res, next) => {
  try {
    ...
    if (req.user.id === order.userId) {
      ...
      return res.status(403).send('The order does\'t belong to current loggedIn customer')
    }
  } catch (err) {
...
const checkRestaurantExists = async (req, res, next) => {
  try {
    ...
    return res.status(409).send('The restaurant does\'t exists')
  } catch (err) {
    return res.status(500).send(err.message)
  }
}

const checkOrderOwnership = async (req, res, next) => { // Hecho en clase
  try {
    const order = await Order.findByPk(req.params.orderId, {
      include: {
        model: Restaurant,
...
const checkOrderVisible = (req, res, next) => { // Hecho en clase
