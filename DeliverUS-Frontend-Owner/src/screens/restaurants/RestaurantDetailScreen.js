...
export default function RestaurantDetailScreen ({ route }) {
  ...
  const productCompararator = function poductComparator (pareja) {
    ...
    if (pareja.value === 0) {
      productOrderObj = Object.fromEntries(
        Object.entries(productOrderObj).filter(([key, value]) => value !== 0)
      )
    }
    console.log(productOrderObj)
    productOrderList = Object.entries(productOrderObj).map(([key, value]) => ([[key], value]))
